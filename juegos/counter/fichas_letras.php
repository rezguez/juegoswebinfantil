...
<div class='container-fluid'>
	...
	<div class='mainbox  mx-auto'>
		...
		<span	class='col titulobarra text-center' onclick='openFullscreen()'
				style='cursor: pointer; margin-bottom: 0px;'>RECORTA Y PEGA LAS LETRAS EN SU LUGAR</span>
		</div>
		...
		<div id='canvas'
			class='row justify-content-center align-items-center mt-3'>
				...
				<div id='idimagen' class='col-4 mx-auto'>
					<img class='img-fluid celdaimagen'
...
for ($i = 0; $i < $numerototal; $i ++) {
    echo "<div name='" . $palabra[$i] . "' id='caja-" . $palabra[$i] . "-" . $i . "'><span class='celda fichascrabble letrascrabble-lg bg-light mx-2' style='opacity:0.4;'>" . $palabra[$i] . "</span></div>";
}
?>

			</div>
			<div id='canvas1'
					class='row  justify-content-center align-items-center mt-5' style='min-height: 7vw; border-top-style:dashed;'>
<?php

for ($i = 0; $i < $numerototal; $i ++) {
    echo "<div name='" . $repalabra[$i] . "' id='caja-" . $repalabra[$i] . "-" . $i . "'><span class='celda fichascrabble letrascrabble-lg bg-light mx-3'>" . $repalabra[$i] . "</span></div>";
}
...
function pintamoviles(value, index) {
		let txto = "";
		//document.getElementById("canvas1").innerHTML += "<div class=' celda c7'  onclick=\"habla('"+value+"')\" id='myelemento-" + value + "-" + index + "' style='left:" + calculaposini(index) + "vw; filter: drop-shadow(5px 5px 5px gray)'>"+txto+"<span class='letrascrabble-lg' style='cursor: pointer'>"+value+"</span></div>";
	}

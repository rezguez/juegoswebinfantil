...
$colores = Array( 'green', 'orange', 'magenta', 'blue','yellow', 'grey',  'red',  'brown');
$formas = Array(  'tri',  'cua',   'rec',  'cir',  'rom');
shuffle($colores);
shuffle($formas);

$azarcolores = array( 1, 2, 3);
$azarformas = array( 1, 2, 3, 4);
	...
	<div class='mainbox  mx-auto'>
		...
		<div class='row p-2 vin justify-content-center mt-0'>
			<a class="col-2  bertateka ml-1" href="index.php">Bertateka</a><span
				...
				onmouseover="habla('ORDENA LAS FORMAS')"
				style='cursor: pointer; margin-bottom: 0px;'>ORDENA LAS FORMAS</span>
		</div>
		<div class='row justify-content-center'>
			<div id='canvas' class='col'>
<?php
for ($x = 0; $x < count($azarcolores); $x ++) {
    for ($y = 0; $y < count($azarformas); $y ++) {
        echo "<div class='movilidad c9 celdainvisible '  id='myelemento-" . $x . "-" . $y . "' style='left:" . ($y * 7) . "vw; top:" . (15 - ($x * 7)) . "vw; filter: drop-shadow(5px 5px 5px gray)'>";
        echo pintaforma($formas[$azarformas[$y]], $colores[($azarcolores[$x])]) . "</div>";
    }
}
?>
</div>
			<div class='col-8 mr-1  '>
				<div class='row align-items-center'>
					...
					<div class='c9 celda m-3 sombra text-center'><?php echo  pintaforma($formas[$azarformas[0]], 'none'); ?></div>
					<div class='c9 celda m-3 sombra text-center'><?php echo  pintaforma($formas[$azarformas[1]], 'none'); ?></div>
					<div class='c9 celda m-3 sombra text-center'><?php echo  pintaforma($formas[$azarformas[2]], 'none'); ?></div>
					<div class='c9 celda m-3 sombra text-center'><?php echo  pintaforma($formas[$azarformas[3]], 'none'); ?></div>
				</div>
<?php
for ($i = 0; $i < count($azarcolores); $i ++) {
    echo "<div class='row justify-content-right align-items-center'>
<div class='c9 celda m-3 sombra text-center' style='background-color:" . $colores[($azarcolores[$i])] . "' ></div>
<div id='caja-" . $i . "-0' class='c9 celda m-3 sombra text-center' style='border-color:" . $colores[($azarcolores[$i])] . "'  ></div>
<div id='caja-" . $i . "-1' class='c9 celda m-3 sombra text-center' style='border-color:" . $colores[($azarcolores[$i])] . "'  ></div>
<div id='caja-" . $i . "-2' class='c9 celda m-3 sombra text-center' style='border-color:" . $colores[($azarcolores[$i])] . "'  ></div>
<div id='caja-" . $i . "-3' class='c9 celda m-3 sombra text-center' style='border-color:" . $colores[($azarcolores[$i])] . "'  ></div>
</div>";
}
?>
</div>
...
for (z=0;z<3; z++) { xleft[z]=z;}
for (z=0;z<4; z++) { xtop[z]=z;}

lista.sort(function(a, b){return 0.5 - Math.random()});
xleft.sort(function(a, b){return 0.5 - Math.random()});
xtop.sort(function(a, b){return 0.5 - Math.random()});

	var moviles = document.getElementsByClassName("movilidad");
	j=0;
for (i=0; i<3; i++) {
	for (t=0; t<4; t++) {
		moviles[lista[j]].style.left=xleft[i]*10 + "vw";
		moviles[lista[j]].style.top=xtop[t]*10 + 5 + "vw";
		j++
	}
}
...
function hacerdragables(value, index) {
    var elmnt = document.getElementById(value.id);
	dragElement(elmnt);
